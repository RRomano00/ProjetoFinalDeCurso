# Fala, Cidade! 🏙️

**Plataforma web para reporte de incidentes urbanos e gestão de zeladoria pública.**

O **Fala, Cidade!** é uma solução digital desenvolvida para estreitar a comunicação entre os cidadãos de Santa Rita do Sapucaí e a administração municipal. Através desta plataforma, munícipes podem registrar ocorrências de infraestrutura (como buracos, iluminação e limpeza) de forma identificada ou anônima, permitindo que a prefeitura realize uma gestão eficiente e baseada em dados.

---

## 🚀 Tecnologias Utilizadas

### Backend
* **Java 21** com **Spring Boot 3**
* **Spring Security** & **JWT** (Autenticação e Autorização)
* **PostgreSQL 18** (Banco de Dados Relacional)
* **API RESTful**

### Frontend
* **Angular 21**
* **Progressive Web App (PWA)** (Suporte para instalação em dispositivos móveis)
* **TypeScript** & **Tailwind CSS**

### Ferramentas de Gestão e Design
* **Git** (Versionamento)
* **Scrum** (Metodologia Ágil)
* **Draw.io** (Modelagem de Dados e Processos)

---

## 📋 Funcionalidades Principais

* **Cidadão:**
  * Registro de notificações com geolocalização e fotos.
  * Acompanhamento de status via número de protocolo único.
  * Opção de envio anônimo (Conformidade com LGPD).

* **Administração Municipal:**
  * Painel de Gestão (Dashboard) com indicadores de demandas.
  * Gestão de usuários (Funcionários e Administradores).
  * Atualização de status e relatórios de desempenho por categoria.

---

## 🛠️ Como Executar o Projeto

### Pré-requisitos
* JDK 21
* Node.js & Angular CLI
* PostgreSQL 18

### Passos
1. **Clonar o repositório:**
   git clone [https://github.com/RRomano00/ProjetoFinalDeCurso.git](https://github.com/RRomano00/ProjetoFinalDeCurso.git)
2. Configurar o Backend:
 * Acesse a pasta /backend.
 * Configure as credenciais do banco no arquivo application.properties.
 * Execute o projeto via Maven ou sua IDE de preferência.

3. Configurar o Frontend:

 * Acesse a pasta /frontend.
 * Execute npm install para as dependências.

 * Execute ng serve para iniciar o servidor de desenvolvimento.

👥 Equipe
Rodrigo Pereira Romano - Desenvolvedor / Documentação

Miguel Martins Lima - Desenvolvedor / Documentação

Lucas Fiorrigi Furlan - Desenvolvedor / Documentação

-----------------------------

📄 Licença
Este projeto é parte integrante do Projeto Final de Curso (PFC) em Sistemas de Informação - FAI (2026).
